...
class ZSignBoardView: UIImageView {
  ...
  func changeImage(){
    ...
    // 如果仅仅点击一下
    if len == 0 {
      let zeroPath = UIBezierPath(arcCenter: points[1], radius: 0.1, startAngle: 0, endAngle: CGFloat(M_PI)*2.0, clockwise: true)
    ...
    // 获取贝塞尔点集
    let curvePoints = ZSBezierPath.curveFactorization(tempPoint1, toPoint: tempPoint2, controlPoints: [points[1]], count: len)
